...
<html>
    <head>
     ...
     <%
        ...
        String saveDir = application.getRealPath("document");
         File f = new File(saveDir);
         f.mkdirs(); 
        String s=session.getAttribute("uid").toString();
        mpr = new MultipartRequest(request, application.getRealPath("document"), limit);
       String t=mpr.getParameter("filetype");
        System.out.print("t="+t);    
       String w=mpr.getOriginalFileName("upload");
        int rs=st.executeUpdate("insert into SHAREFILES values('"+s+"','document','"+w+"')");
    if(rs>0)
