...
<html>
    <head>
        ...
        int limit=1024*1024*1024;
         String saveDir = application.getRealPath("audio");
         File f = new File(saveDir);
         f.mkdirs(); 
        String s=session.getAttribute("uid").toString();
        mpr = new MultipartRequest(request, application.getRealPath("audio"), limit);
       String t=mpr.getParameter("filetype");
        System.out.print("t="+t);    
         String w=mpr.getOriginalFileName("upload");
        int rs=st.executeUpdate("insert into SHAREFILES values('"+s+"','audio','"+w+"')");
    if(rs>0)
